#!/usr/bin/env python3
"""Версия 5: планы БЕЗ подписей (их накладываем поверх готового рендера,
...
Запуск:
    python3 render_phone_v5.py <ключ>                  # обе комнаты, medium
    python3 render_phone_v5.py <ключ> bedroom          # только спальня
    OPENAI_IMAGE_QUALITY=high python3 render_phone_v5.py <ключ> bedroom
...
def main() -> int:
    ...
    if not api_key:
        print("Укажите ключ: python3 render_phone_v5.py <ключ> [комната …]", file=sys.stderr)
        return 1
