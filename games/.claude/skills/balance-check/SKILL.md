...
Read relevant files from `games/<game>/src/` and `games/<game>/src/` for the identified domain.
...
Read the GDD for the system from `docs/` to understand intended design targets,
...
- Options:
  ...
  - `[B] Save report to games/<game>/src/balance-check-[system]-[date].md`
...
- Guide the user to update the relevant data file in `games/<game>/src/` or formula in `games/<game>/src/`
...
If [B]:
- Write the report to `games/<game>/src/balance-check-[system]-[date].md` (create the directory if needed). Use the current date for [date] in YYYY-MM-DD format.
