...
- Compare against the existing string table in `games/<game>/src/strings/`
...
Present the diff to the user. Ask: "May I write these new entries to `games/<game>/src/strings/strings-en.json`?"
...
Read all string table files in `games/<game>/src/strings/`. For each locale, check:
...
Read:
- `docs/` — extract game genre, tone, setting, character names
- `games/<game>/src/strings/strings-en.json` — the source string table
...
Spawn `localization-lead` via Task. Ask them to audit the following for cultural sensitivity across the target locales (read from `games/<game>/src/strings/` and `assets/`):
...
Read `games/<game>/src/strings/` and `design/narrative/`. Identify:
...
- The list of all screens/flows in the game (from `docs/` or `/content-audit` output)
