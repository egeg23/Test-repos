...
- Search `docs/` for the GDD most relevant to this change. Read the
  sections that this change would affect.
- Check whether `docs/systems-index.md` exists. If it does, read it to
...
- If this is a Tuning change, also check `games/<game>/src/` for the data file that
...
**Type**: Tuning
**System**: [System name]
**GDD Reference**: `docs/[filename].md` — Tuning Knobs section
**Date**: [today]
...
- [ ] [Parameter] reads [new value] from `games/<game>/src/[file]`
...
**Type**: [Tweak / Addition]
**System**: [System name]
**GDD Reference**: `docs/[filename].md`
**Date**: [today]
...
Current GDD says (quoting `docs/[filename].md`, [section]):
...
All values must live in `games/<game>/src/[appropriate-file].json`, not hardcoded.
...
This system is not currently in `docs/systems-index.md`.
...
`docs/[filename].md` — specifically the [section name] section?"
