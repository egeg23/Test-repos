...
Read `docs/game-concept.md`. If it does not exist, fail with:
...
**Retrofit mode detection**: Glob `docs/art/art-bible.md`. If the file exists:
- Read it in full
...
- Present this table to the user:
  > "Found existing art bible at `docs/art/art-bible.md`. [N] sections are complete, [M] need content. I'll work on the incomplete sections only — existing content will not be touched."
...
- Does `docs/systems-index.md` exist? → map-systems is done, skip that option
...
- Does `docs/` contain any `*.md` files? → design-system has been run, skip that option
- Does `docs/gdd-cross-review-*.md` exist? → review-all-gdds is done
