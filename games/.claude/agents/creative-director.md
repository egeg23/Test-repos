...
skills: [quick-design, design-review]
---
...
Let me read:
 - docs/crafting-system.md (what's designed)
 - docs/sprints/sprint-current.md (what's planned)
...
2. Updating docs/crafting-system.md with Alpha scope markers
3. Notifying game-designer and lead-programmer to align on new 1.5-week plan
4. Adding to docs/milestones/alpha.md as success criteria: 'Crafting pillar demonstrated via core discovery mechanic'
...
- Соло-разработка: делегировать некому, роли `writer`, `game-designer`,
  `art-director` в проекте отсутствуют. Их работу выполняет тот же исполнитель.
- Ванильный JS без движка, растровых ассетов нет: весь визуал рисуется кодом.
  Нарратив не может опираться на иллюстрации, катсцены или озвучку.
- Игра на 4–8 недель. Объём текста — десятки строк, а не сценарий.
- Платформа Яндекс Игры: аудитория 45+ около половины, женщин 58%.
  Требования платформы к контенту (§3.4) имеют приоритет над замыслом.
